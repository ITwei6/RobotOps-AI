...
- `/diagnose` 返回 `evidence_claims`，每条结论带 `evidence_refs`、`support_level` 和 `confidence`；引用不在当前报告证据集合中的 ref 会被清洗掉。
...
- Agent 在工具结果没有新增证据时停止重复查询，但会先完成尚未尝试的案例和知识检索；评测输出 `claim_grounding_rate` 和 `claim_evidence_coverage`。
