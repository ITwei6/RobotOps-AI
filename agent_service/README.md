...
- `langgraph-diagnosis-v2` 工作流，内部按 `normalize_input -> rule_evidence -> planner -> tool_executor -> observation_analyzer -> source_analysis -> report -> confidence_check -> finalize` 执行。
...
- 增加符号索引、文件摘要和调用关系索引，提升大仓库中的候选排序与上下文扩展能力。
