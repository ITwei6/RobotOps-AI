...
```text
面向机器人研发测试阶段和售后运维阶段的一体化 AI 诊断平台。
```
...
| 售后运维阶段 | 支持机器人卖出后的远程运维 | 实时状态、模块心跳、事件告警、远程日志、售后工单 |
...
- `robot-gateway`：接入机器人侧日志包、状态、心跳、事件。研发阶段处理日志包导入，售后阶段处理实时上报。
- `log-service`：日志解析、日志索引、日志检索、时间窗口上下文。
- `ticket-service`：研发 Bug 单和售后工单。
...
- 研发阶段日志包分析是核心，售后阶段实时监控是扩展，不要把二者割裂。
