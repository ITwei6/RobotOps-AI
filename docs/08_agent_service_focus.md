...
1. 根据首轮源码函数中的被调函数、RPC/Topic 和接口类型执行受控的二次检索。
2. 增加符号索引、文件摘要和调用关系索引，支持大仓库上下文扩展。
3. 将真实 interaction、mc、hal、hds、sm、agent Bug 诊断过程沉淀为可检索历史案例。
4. 将本地案例和知识文件扩展为 knowledge-service / RAG。
5. 增加 LangGraph checkpoint、诊断轨迹持久化和人工复核反馈。
